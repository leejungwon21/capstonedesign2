...
홈 종목 행은 `/stock/005930` 등의 독립 주소로 연결됩니다. 상세 페이지는 핵심 분석(근거/시점별 관계망), 수급 흐름(차트/표), 공시·경보 이력 탭으로 구성했습니다. 관계망은 예시 수급을 그린 것으로 GNN 결과가 아닙니다. DART는 미연결 상태를 표시합니다. 업로드된 실제 이력은 종목코드로 매칭하되 예시 수급과 연관된 분석으로 표시하지 않습니다.
